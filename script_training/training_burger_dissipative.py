...
from pinn_loss.loss_operator import BurgerLossDissipative
from pinn_loss.utils import create_graph_burger, create_burger_dataset
...
def init_model_gnn(dataloader, delta_t=0.01, index_edge_derivator=0, index_node_derivator=0):
    ...
    burger_loss = BurgerLossDissipative(delta_t=delta_t, index_edge_derivator=index_edge_derivator, index_node_derivator=index_node_derivator, mu=0.001)
...
def main_train():
    ...
    dataloader = DataLoader(dataset, batch_size=config_trainer["batch_size"], shuffle=True, num_workers=0)
...
def main_eval():
    ...
    edges, edges_index = create_graph_burger(nb_space, delta_x=delta_x)
